...
app.get("/", (req, res) => {
  res
    .status(200)
    .send(
      "Hello there!<br> Access /student for all student details <br> Access /mentor for all mentor details"
    );
});
...
app.put("/assign-change-mentor/:id", async (req, res) => {
  if (req.params.id.length === 24 && req.body.mentor.length === 24) {
    try {
      let client = await mongoClient.connect(dbURL);
      let db = client.db("myDB");
      let student_check = await db
        .collection("student")
        .findOne({ _id: objectId(req.params.id) });
      let mentor_check = await db
        .collection("mentor")
        .findOne({ _id: objectId(req.body.mentor) });
          ...
          if (req.body.mentor == student_check.mentor) {
            ...
            if (mentor_check.student) {
              ...
              await db
                .collection("mentor")
                .updateOne(
                  { _id: objectId(req.body.mentor) },
                  { $push: { student: req.params.id } }
                );
            } else {
              ...
              await db
                ...
                .updateOne(
                  { _id: objectId(req.body.mentor) },
                  { $set: { student: [req.params.id] } }
                );
            }
            await db
              .collection("mentor")
              .updateOne(
                { _id: objectId(old_m_id) },
  ...
  try {
    ...
    if (data) {
      if (data.length > 0) res.status(200).json(data);
        ...
        res
          .status(404)
          .send("No data found or all studends have a mentor assigned!");
    } else {
      res.status(404).json({
  ...
  if (req.params.id.length === 24 && sid_check) {
    try {
      ...
      for (var i = 0; i < req.body.student.length; i++) {
        var check = await db
          .collection("student")
          .findOne({ _id: objectId(req.body.student[i]) });
      ...
      if (mentor_check && student_check) {
        if (mentor_check.student) {
          ...
          await db
            ...
            .updateOne(
              { _id: objectId(req.params.id) },
              { $push: { student: { $each: req.body.student } } }
            );
        } else {
        ...
        var output =
          "Students added to mentor!<br> And mentor added to student" + result;
        ...
        res
          .status(404)
          .send(
            "Please check the student and Mentor IDs. One or more student ID or Mentor ID is not found!"
          );
      }
      client.close();
    } catch (error) {
    ...
    res
      .status(400)
      .send("Either mentor id or any of the student ID is invalid!");
  }
});
app.listen(port, () =>
  console.log("App index.js inside Task is running on port:", port)
);
